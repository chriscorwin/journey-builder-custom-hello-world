...
var growBy = function growBy(numberToGrowBy) {
	var numberToGrowByReally = numberToGrowBy || 1;
	$(".canvas-view-swim-lane-header").each(function(index, element) {
		var newLeftBase = 160;
		if ($(this).position().left > 0) {
			console.debug("$(this)", $(this));
			console.log($(this).position().left);
			var newLeft = ((newLeftBase * index) + (newLeftBase * numberToGrowByReally));

			$(this).css({
				left: newLeft + 'px',
				width: ((newLeftBase - 1) + 'px')
			});
		} else {
			$(this).css({
				width: (((newLeftBase * (numberToGrowByReally + 1)) - 1) + 'px')
			});
		}
	});
}
