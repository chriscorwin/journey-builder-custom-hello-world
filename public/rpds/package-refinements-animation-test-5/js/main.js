...
var init = function init() {
	console.group('Initializing RPD now...');
	$('.draggable').each(function(i, el) {
		// $('.draggable:not(".dropson")').each(function(i, el) {
		$el = $(el);


		var clone = $el.attr('data-drag-clone-img') || 'clone';
		var cloneImg = clone;
		var thisClone = clone,
			thisCloneID = "",
			thisCloneClassName = $el.attr('class'),
			thisImagesDirectoryName = "",
			$thisClone, showClone = function() {
				return
			}, hideClone = function() {
				return
			};
		...
		console.log("$el.attr('data-drag-clone-img')", $el.attr('data-drag-clone-img'));
		if (clone !== 'clone') {
			// clone = function() {
			// 	return '<img src="images/' + cloneImg + '"/>';
			// };
			if (typeof clone === "string" && clone.beginsWith("#") === false && clone.beginsWith(".") === false) {
				// assume it's an actual image uploaded into the images/ directory
				clone = function() {
					return '<img src="images/' + cloneImg + '"/>';
				};

			} else if (typeof clone === "string" && clone.beginsWith("#") === true) {
				clone = 'original';
				// clone = function() {
				// 	return '<img src="images/' + cloneImg + '" class="' + thisCloneClassName + '">';
				// 	// return $thisClone;
				// };
				showClone = function() {
					$thisClone = $(clone);
					thisCloneID = $thisClone.attr("id");
					cloneImg = $thisClone.css("background-image");
					...
					debug.debug("cloneImg", cloneImg);
					// $el.css('background', cloneImg);
					// debug.debug("images/" + thisImagesDirectoryName + '/' + cloneImg);
					// $(el).css('width', '200px');
				};
				hideClone = function() {
					$thisClone = $(clone);
		...
		$el.draggable({
			...
			start: function(event, ui) {
				...
				console.log("$el.attr('data-drag-clone-img')", $el.attr('data-drag-clone-img'));
				console.log('typeof clone', typeof clone);
	...
	$(".droppable:not('.rpdDropTarget')").each(function(i, el) {
