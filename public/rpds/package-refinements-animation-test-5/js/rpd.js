...
var showDeep = function showDeep(el) {
	...
	$thisElement.trigger('show').velocity("fadeIn", {
		stagger: 100,
		drag: true,
		duration: 125
	}, {
	...
	$hiddenNodes = $thisElement.find(".hidden:not(.startshidden)");
	show($hiddenNodes);
...
var hideState = function hideState(el) {
	...
	$thisElement.velocity("transition.slideLeftOut", {
		stagger: 100,
		drag: true,
		duration: 125
	}, {
...
var slideJbDays = function slideJbDays() {
	...
	var zeroNewWidth = (zeroWidth + activityWidth)
	...
	// 	debug.debug("$otherJbDays", $otherJbDays);
	...
	// $otherJbDays.each(function(index) {
	// 	console.log($(this).position().left)
	// 	var thisWidth = $(this).width()
	// 	var newLeft = $(this).position().left + (activityWidth)
	// 	$(this).velocity({
	// 		left: newLeft
	// 	}, 125)
	// })
...
var runIt = function runIt() {
	...
	$(".state:not(.shared)").on("visiting", function(event) {
		...
		show($(".icon." + thisStateRoot + ":not(.visited)"))
		// $(".icon." + thisStateRoot + ":not(.visited)").hasClass('pleaseHide').show();
	...
	$(".state").on("configure", function(event) {
		...
		show($(".icon." + thisStateRoot + ".configured"))

		// $(".icon." + thisStateRoot + ".configured").hasClass('pleaseHide').show();
	});
	...
	$('[data-event-onshow]').on('show', function(event) {
		console.debug("shown!");
	...
	if ($('[data-name="jbDays"]').length != 0) {
		...
		debug.debug("activityWidth", activityWidth);
		debug.debug("zeroWidth", zeroWidth);
		debug.debug("zeroNewWidth", zeroNewWidth);
