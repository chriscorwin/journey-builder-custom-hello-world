define(function(require) {
	...
	connection.on('requestPayload', function() {
		...
		payload.flowDisplayName = 'Update Contact';
...
var editImageCallback = function editImageCallback(e) {
	...
	.attr("src", 'img/' + context.elmImgFileName)
		.load(function() {
...
var editGroupCallback = function editGroupCallback(e) {
	...
	var bgImgFile = getFileProperties(groupForm, 'bgimg');
...
var init = function init() {
	...
	$('.hoverable').hover(function(e) {
		$(e.target).attr('src', 'img/' + $(e.target).data('hover'));
	}, function(e) {
		$(e.target).attr('src', 'img/' + $(e.target).data('elm-img-file'));
	});
	$('style', '#root').appendTo('head');
...
$.fn.preload = function() {
	this.each(function() {
		...
		$('<img />').attr('src', 'img/' + this).appendTo('body').css('display', 'none');
	});
}
